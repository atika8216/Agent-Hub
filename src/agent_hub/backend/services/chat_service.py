...
def stream_chat(
    ...
    # (AGENT_HUB_DISABLE_UC_MCP_CHAT=1) reverts to the Phase-1 friendly stub
    # so we can roll back without a code revert.
    if _is_uc_connection(endpoint_name) or _is_mcp_endpoint(endpoint_name):
        if os.environ.get("AGENT_HUB_DISABLE_UC_MCP_CHAT") == "1":
            ...
            stub_text = (
                ...
                "disabled (AGENT_HUB_DISABLE_UC_MCP_CHAT). Please contact the "
                "workspace admin."
            )
...
def _check_uc_execute_privilege(
    ...
    """
    warehouse_id = (
        os.environ.get("AGENT_HUB_USER_WAREHOUSE_ID")
        or os.environ.get("DATABRICKS_WAREHOUSE_ID")
        or ""
    ).strip()
...
def _stream_http_connection(
    ...
    if not warehouse_id:
        msg = (
            "UC HTTP chat requires AGENT_HUB_ADMIN_WAREHOUSE_ID (or "
...
def _post_stream(
    ...
    try:
        ...
        resp._agent_hub_client = client  # type: ignore[attr-defined]
        return resp
    except Exception:
        client.close()
        raise
...
def _close_stream(resp: httpx.Response | None) -> None:
    ...
    try:
        ...
        pass
    client = getattr(resp, "_agent_hub_client", None)
