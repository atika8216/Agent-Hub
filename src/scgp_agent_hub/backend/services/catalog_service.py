...
# functions or connections with a role tag (default key ``scgp_agent_hub_role``
# value ``agent``) and an optional kind tag (default key ``scgp_agent_hub_kind``
...
def _admin_warehouse_id() -> str:
    ...
    Precedence: ``SCGP_ADMIN_WAREHOUSE_ID`` (explicit config) ->
...
def _discover_uc_tagged(
    ...
    if not warehouse_id:
        warnings.append(
            "UC tag discovery skipped: set SCGP_ADMIN_WAREHOUSE_ID "
            "(or DATABRICKS_WAREHOUSE_ID) to enable"
        )
        logger.info("UC tag discovery skipped: no admin warehouse configured")
        return 0, 0, 0, warnings

    if os.environ.get("SCGP_DISABLE_UC_MCP_DISCOVERY") == "1":
        logger.info("UC tag discovery disabled via SCGP_DISABLE_UC_MCP_DISCOVERY")
        return 0, 0, 0, warnings
